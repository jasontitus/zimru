...
fn write_minimal_corpus(dir: &std::path::Path) {
    std::fs::write(
        dir.join("index.html"),
    ...
    let png: &[u8] = &[
        ...
        0x15, 0xc4, 0x89, 0x00, 0x00, 0x00, 0x0d, b'I', b'D', b'A', b'T', 0x78, 0x9c, 0x63, 0xfc,
        0xff, 0xff, 0x3f, 0x03, 0x00, 0x06, 0x00, 0x02, 0xfe, 0xa6, 0x88, 0x73, 0x88, 0x00, 0x00,
...
fn builds_indexes_when_xapianbuilder_available() {
    ...
    assert!(
        entries.contains(&"fulltext/xapian".to_string()),
        "missing X/fulltext/xapian; got {entries:?}"
    );
    assert!(
        entries.contains(&"title/xapian".to_string()),
